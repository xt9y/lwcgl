...
header-check: | $(TEST_DIR)
	@set -e; for h in $(PUBLIC_HEADERS); do n=$$(basename $$h .h); printf '#include <lwcgl/%s.h>\nint main(void){return 0;}\n' $$n > $(TEST_DIR)/header-$$n.c; $(CC) $(CPPFLAGS) $(CFLAGS) -Werror -c $(TEST_DIR)/header-$$n.c -o $(TEST_DIR)/header-$$n-c.o; printf '#include <lwcgl/%s.h>\nint main(){return 0;}\n' $$n > $(TEST_DIR)/header-$$n.cpp; $(CXX) $(CPPFLAGS) $(CXXFLAGS) -Werror -c $(TEST_DIR)/header-$$n.cpp -o $(TEST_DIR)/header-$$n-cpp.o; done
